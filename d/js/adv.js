(function (j, K, g) {
	function l(a) {return Object.prototype.toString.call(a) === "[object Array]"}
	...
	var J       = _py.getLast("a"), C = ["R6", "CV", "9b", "g2", "xT", "_d"], y = J && J.split(".")[0], k = true;
	...
	if (k) {
		var x = _py.getLast("p"), I = _py.getLast("pi"), v = _py.getLast("pv"), F = _py.getLast("e"), G = _py.getLast("domain"), B = _py.getLast("mapping"), H = _py.getLast("urlParam");
		(function (i, q, p, f, e) {
			i._CommandName_ = f;
			i[f] = i[f] || function () {
					(i[f].q = i[f].q || []).push(arguments);
					i[f].track = function () {(i[f].q[i[f].q.length - 1].t = []).push(arguments)};
					return i[f]
		...
		j.ipy         = {
			r                : /(^|&)jump=(\d*)/i, cookie: {
				...
				get   : function (c) {return (c = K.cookie.match(RegExp("(^|;)\\s*" + c + "=([^;]*)", "i"))) ? decodeURIComponent(c[2]) : ""}
			}, setCookie     : function (c, a) {ipy.cookie.set(c, a, 31536000000, A())},
			setSession       : function (c, a) {ipy.cookie.set(c, a, 0, A())}, getJump: function () {
		...
		var x = location.href, r = K.referrer, F, I, t;
		j.parent != j && (x = r, r = "");
		x && _py.push(["u", x]);
		r && _py.push(["r", r]);
		var n = _py.getLast("urlParam") || "pyck", G = ipy.getQueryString(n);
